...
import { useToggleLike } from "@/features/post/hooks";
      ...
      <div className="px-4 sm:py-5 py-4">
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Post Content */}
            <p className="text-[15px] mb-2.5! text-foreground">{content}</p>
            ...
            {media_urls && media_urls.length > 0 && (
              <div
                className={cn(
                  ...
                  media_urls.length === 1 && "grid-cols-1",
                  media_urls.length === 2 && "grid-cols-2",
              ...
              >
                {media_urls.map((img, index) => (
                  <img
                    ...
                    className={cn(
                      "w-full object-cover",
                      media_urls.length === 1 ? "h-80 max-h-[500px]" : "h-56",
                    )}
                  />
                ))}
              </div>
              ...
              <div className="mb-3 border border-border rounded-xl p-3 bg-card/50">
                <div className="flex gap-2 items-start">
                  <Avatar className="w-6 h-6 shrink-0">
                    <AvatarImage
                      src={original_post.user?.avatar_url}
                      alt={original_post.user?.username}
                    />
                    <AvatarFallback className="bg-secondary text-foreground text-xs">
                      {original_post.user?.username?.charAt(0).toUpperCase() ||
                        "U"}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-foreground-secondary line-clamp-3 my-1.5!">
